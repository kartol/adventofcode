...
class Directory:
    ...
    def du(self):
        return sum([each.du() for each in [*self.files.values(), *self.directories.values()]])

    def list_dirs(self):
        return [d.du() for d in self.directories.values()] + [d.list_dirs() for d in self.directories.values()]
...
class FS:
    ...
    def list_dirs(self):
        return self.root_directory.list_dirs()
...
print(sum([each for each in flatten(fs.list_dirs()) if each < 100000]))
